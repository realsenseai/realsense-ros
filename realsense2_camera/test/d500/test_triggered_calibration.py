...
def test_triggered_calibration():
    ...
    tester = Tester()
    try:
        tester.set_integer_param('safety_camera.safety_mode',2)
        time.sleep(0.5)
        LOGGER.info("Param safety_camera.safety_mode: %s", str(tester.get_integer_param('safety_camera.safety_mode')))
        # switch to visual preset #1
        tester.set_integer_param('depth_module.visual_preset',1)
        time.sleep(0.5)
        LOGGER.info("Param depth_module.visual_preset: %d", tester.get_integer_param('depth_module.visual_preset'))

        # enable emitter
        tester.set_bool_param('depth_module.emitter_enabled',True)
        time.sleep(0.5)
        LOGGER.info("Param depth_module.emitter_enabled: %d", tester.get_bool_param('depth_module.emitter_enabled'))

        # enable auto exposuretc_done   CAMERA_NAME,
        tester.set_bool_param( 'depth_module.enable_auto_exposure',True)
        time.sleep(0.5)
        LOGGER.info("Param depth_module.enable_auto_exposure: %s", tester.get_bool_param('depth_module.enable_auto_exposure'))

        # turn off depth streaming
        tester.set_bool_param('enable_depth',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_depth: %s", tester.get_bool_param('enable_depth'))
        
        # turn off infra1 streaming
        tester.set_bool_param('enable_infra1',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_infra1: %s", tester.get_bool_param('enable_infra1'))
        # turn off infra2 streaming
        tester.set_bool_param('enable_infra2',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_infra2: %s", tester.get_bool_param('enable_infra2'))
        # turn off safety streaming
        tester.set_bool_param('enable_safety',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_safety: %s", tester.get_bool_param('enable_safety'))
        # turn off occupancy streaming
        tester.set_bool_param('enable_occupancy',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_occupancy: %s", tester.get_bool_param('enable_occupancy'))

        tester.set_bool_param('enable_color',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_color: %s", tester.get_bool_param('enable_color'))

        # turn off lpcl streaming
        tester.set_bool_param('enable_labeled_point_cloud',False)
        time.sleep(0.5)
        LOGGER.info("Param enable_labeled_point_cloud: %s", tester.get_bool_param('enable_labeled_point_cloud'))
        tester.send_goal()

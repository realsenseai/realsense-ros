...
def launch_setup(context, params, param_name_suffix=''):
    ...
    # Use Node directly with manual parameter overrides
    node_action = launch_ros.actions.Node(
        ...
        name=LaunchConfiguration('camera_name' + param_name_suffix),
        parameters=[
            params,              # LaunchConfig parameters
            params_from_file,    # Any YAML file parameters
            {   # Manual overrides
                'rgb_camera.white_balance': 4600.0,  # note the decimal
                'rgb_camera.exposure': 2321,       # safer to make float
                'depth_module.exposure': 8500.0,
                'depth_module.gain': 16.0,
    ...
    return [LogInfo(msg="🚀 Launching RealSense Camera Node with manual exposure/white balance"), node_action]
